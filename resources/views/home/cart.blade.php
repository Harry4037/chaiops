...
<section class=cartpage>
    <div class=container>
        <div class="cartHeader clearfix">
            <h1 class=logo> <a href="/"> <img src="{{ asset("assets/images/logo.png") }}"
                        alt=image> </a> </h1>
            <div class=orderId>
                <div>
                    <p class=datetime></p>
                ...
                @if(session('cart'))
                    @foreach(session('cart') as $id => $product )
                        ...
                        <li class="clearfix">
                            <div class="item-content">
                                ...
                                <div class="item-details">
                                    ...
                                    <div class="productQuantity"><select class="form-control update-cart">
                                            <option value="1" data-id="{{ $id }}" @if($product['quantity']==1) selected
                                                @endif>1 product
                                            </option>
                                            <option value="2" data-id="{{ $id }}" @if($product['quantity']==2) selected
                                                @endif>2 product
                                            </option>
                                            <option value="3" data-id="{{ $id }}" @if($product['quantity']==3) selected
                                                @endif>3 product
                                            </option>
                                            <option value="4" data-id="{{ $id }}" @if($product['quantity']==4) selected
                                                @endif>4 product
                                            </option>
                                        </select></div>
                                    <div></div>
                            ...
                            <a href="#" class="remove-from-cart removeProduct" data-id="{{ $id }}"><i
            ...
            @elseif(session()->get('cart'))
                <div class="clearfix order-box">
                    ...
                    <div class=selected-item-no>
                        <p>You have selected <span>{{ count(session()->get('cart')) }}</span> items
                        </p>
                    </div>
